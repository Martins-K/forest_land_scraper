...
export async function sendEmail(
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        user: process.env.EMAIL_USER_FOREST,
        pass: process.env.EMAIL_PASS_FOREST,
      },
    });
...
<head>
    <meta charset="UTF-8">
    <title>Forest Scraper Report</title>
</head>
<body>
    <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
      ...
      newItems.length
        ...
        <h3 style="color: #0066cc;">New Forest Listings</h3>
        ${itemsTable}
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #ddd;">
            <p style="color: #666;">
                At your service,<br>
                <strong>Forest Scraping Service</strong>
            </p>
        </div>
    </div>
</body>
    ...
    const mailOptions = {
      ...
      subject: `Forest Scraper Report - ${new Date().toLocaleDateString()} (${
        newItems.length
      } new items)`,
      html: emailBody,
      attachments: [
        {
          filename: "forests-scraped.xlsx",
          path: fileName,
        },
      ],
    };
